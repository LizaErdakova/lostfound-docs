...
```YAML
asyncapi: 3.1.0
id: 'urn:lostfound:chat:after-owner-check:websocket'
info:
  title: Lost&Found Real-Time Chat API
  version: 1.0.0
  description: >
    Контракт AsyncAPI для чата после успешного подтверждения владельца.
    Изначально диалог создаётся в режиме owner-check. Свободный чат становится доступен только после того, как результат owner-check будет подтвержден.
...
channels:
  '/ws/chat/{dialogId}':
    address: '/ws/chat/{dialogId}'
    messages:
      clientToServerChatEvents.message.0:
        $ref: '#/components/messages/chatJoin'
      clientToServerChatEvents.message.1:
        $ref: '#/components/messages/chatMessageSend'
      serverToClientChatEvents.message.0:
        $ref: '#/components/messages/chatConnected'
      serverToClientChatEvents.message.1:
        $ref: '#/components/messages/chatMessageReceived'
      serverToClientChatEvents.message.2:
        $ref: '#/components/messages/chatError'
    description: >
      WebSocket-канал для одного диалога. Подключение разрешено только аутентифицированным участникам диалога и только в том случае, если диалог переведён из режима owner-check в режим обычного чата.
    parameters:
      dialogId:
...
operations:
  clientToServerChatEvents:
    action: receive
    channel:
      $ref: '#/channels/~1ws~1chat~1{dialogId}'
    summary: Events sent by the client to the server.
    messages:
      - $ref: >-
          #/channels/~1ws~1chat~1{dialogId}/messages/clientToServerChatEvents.message.0
      - $ref: >-
          #/channels/~1ws~1chat~1{dialogId}/messages/clientToServerChatEvents.message.1
  serverToClientChatEvents:
    action: send
    channel:
      $ref: '#/channels/~1ws~1chat~1{dialogId}'
    summary: Events sent by the server to the client.
    messages:
      - $ref: >-
          #/channels/~1ws~1chat~1{dialogId}/messages/serverToClientChatEvents.message.0
      - $ref: >-
          #/channels/~1ws~1chat~1{dialogId}/messages/serverToClientChatEvents.message.1
      - $ref: >-
          #/channels/~1ws~1chat~1{dialogId}/messages/serverToClientChatEvents.message.2
components:
  securitySchemes:
    bearerAuth:
      type: http
  ...
  messages:
    chatJoin:
      name: chat.join
      title: Join chat dialog
      summary: Client requests joining the dialog channel.
      contentType: application/json
      payload:
        $ref: '#/components/schemas/ChatJoinPayload'
      examples:
        - name: joinDialog
          summary: Client connects to a confirmed dialog.
          payload:
            eventType: chat.join
            dialogId: 8e2c60aa-3d39-4b85-a25a-efefaa194e2a
            userId: 1f9e8f1f-56e8-4f90-a7d6-b8c0cc52fd27
            sentAt: '2026-04-19T12:00:00Z'
    chatMessageSend:
      name: chat.message.send
      title: Send chat message
      summary: Client sends a new text message to the dialog.
      contentType: application/json
      payload:
        $ref: '#/components/schemas/ChatMessageSendPayload'
      examples:
        - name: sendTextMessage
          summary: Client sends a text message after owner-check confirmation.
          payload:
            eventType: chat.message.send
            dialogId: 8e2c60aa-3d39-4b85-a25a-efefaa194e2a
            messageId: 7a5f5f60-c5f0-4ff9-bdf0-69de8e1b53e3
            text: Здравствуйте! Можем договориться о передаче вещи?
            sentAt: '2026-04-19T12:00:05Z'
    chatConnected:
      name: chat.connected
      title: Chat connection confirmed
      summary: Server confirms that the client has connected to the dialog.
      contentType: application/json
      payload:
        $ref: '#/components/schemas/ChatConnectedPayload'
      examples:
        - name: connectionAccepted
          summary: Server confirms access to a normal chat dialog.
          payload:
            eventType: chat.connected
            dialogId: 8e2c60aa-3d39-4b85-a25a-efefaa194e2a
            userId: 1f9e8f1f-56e8-4f90-a7d6-b8c0cc52fd27
            dialogStatus: normal_chat
            ownerCheckStatus: confirmed
            connectedAt: '2026-04-19T12:00:01Z'
    chatMessageReceived:
      name: chat.message.received
      title: New message delivered
      summary: >
        Server delivers a newly saved message to both dialog participants,
        including the sender and the recipient.
      contentType: application/json
      payload:
        $ref: '#/components/schemas/ChatMessageReceivedPayload'
      examples:
        - name: deliveredTextMessage
          summary: >
            Server pushes a newly saved message to both participants, including
            the sender.
          payload:
            eventType: chat.message.received
            dialogId: 8e2c60aa-3d39-4b85-a25a-efefaa194e2a
            messageId: 7a5f5f60-c5f0-4ff9-bdf0-69de8e1b53e3
            senderId: 1f9e8f1f-56e8-4f90-a7d6-b8c0cc52fd27
            text: Здравствуйте! Можем договориться о передаче вещи?
            sentAt: '2026-04-19T12:00:05Z'
    chatError:
      name: chat.error
      title: Chat error
      summary: Server reports a message processing error inside an active chat session.
      contentType: application/json
      payload:
        $ref: '#/components/schemas/ChatErrorPayload'
      examples:
        - name: validationError
          summary: Server rejects a message because it does not pass validation.
          payload:
